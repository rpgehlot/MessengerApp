...
import { Enums } from '@/app/lib/database-types';
                    ...
                    <div 
                        ...
                        <div className='text-[10px] font-light text-secondary-foreground -mb-2 min-w-16 text-right'>
                            <span className='inline-block'>
                                {formatDateToLocal(props.createdAt)}
                            </span>
                            <DoubleTick status={props.status} sentByMe={sentByMe}/>
                        </div>
                    </div>
            </div>
            </div>
...
type DoubleTickProps = {
    sentByMe : boolean;
    status : Enums<'messagestatus'>
};

function DoubleTick(props: DoubleTickProps) {

    if (!props.sentByMe)
        return null;

    const color = props.status === 'read' ? '#53bdeb' : '#8696a0';
    return (
        <span className='inline-block align-middle mx-1'>
            {
                props.status!='queued' && (
                        <svg viewBox="0 0 16 11" height="11" width="16" preserveAspectRatio="xMidYMid meet" className="" 
                            fill={color}>
